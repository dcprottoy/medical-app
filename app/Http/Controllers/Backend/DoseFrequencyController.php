...
class DoseFrequencyController extends Controller
{
    /**
    ...
    {
        $data['dose'] = DoseFrequency::paginate(5);
        return view('backend.dosefrequency.index',$data);
    }
    ...
    {
        $lastid = DoseFrequency::findOrFail($id);
        return $lastid;
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($validated->fails()){
            ...
            $advice = DoseFrequency::findOrFail($id);
    ...
    {
        if(DoseFrequency::find($id)){
            $createObject = DoseFrequency::find($id);
            $createObject->delete();
    ...
    public function search(Request $request){
        $search = $request->q;
        $diagnosis = DoseFrequency::where('name_eng','LIKE','%'.$search.'%')->get();
        return $diagnosis;
    }
}
